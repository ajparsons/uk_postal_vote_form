...
from .codepointfix import CodePointZipFix
import datetime
import json
	
class Election(FlexiBulkModel):
	"""
	Possible Election Dates - form is pre-popped with date not name. 
	"""
...
class Council(FlexiBulkModel):
	"""
	All ERCs in UK - from Democracy Club APi
	"""
	name = models.CharField(max_length=255, default="", blank=True)
	...
	def populate(cls):
		...
		See Democracy Club API
		https://wheredoivote.co.uk/api/beta/councils.json
		"""
...
class Postcode(FlexiBulkModel):
	"""
	stores reduced vesion of OS code point open for council and postcode connection
	"""
	...
	def get_postcode(cls, postcode):
		...
		try:
			return Postcode.objects.get(postcode=r_postcode)
		except Postcode.DoesNotExist:
			return None
	...
	def populate(cls, delete_current=False):
		...
		codepoint = CodePointZipFix('resources//codepo_gb.zip')
		
		"""
		import lookup of postcodes crossing council boundaries
		"""
		split = QuickGrid().open('resources/split-postcodes.csv')
		split_lookup = {
			reduce(x["postcode"]): int(x["authorities"]) for x in split}
		...
		Postcode.save_queue(5000,retrieve=False)
